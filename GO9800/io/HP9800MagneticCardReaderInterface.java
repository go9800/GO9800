        ...
        case CARD_END_RECORD | CARD_END_LOAD | CARD_IN | CARD_UNPROTECTED:
          if(readMode && asyncMode) {
            ...
            try {
              ...
              ioReg.bus.din = cardFile.readByte() | sensors;

              // set card reader flag
              ioReg.MFL = true;

              if(debug)
                ioReg.console.append("HP9800 MC read async " + Integer.toHexString(ioReg.bus.din & CARD_DATA) + "\n");
            } catch (IOException e) {
  ...
  public boolean output()
  {
    synchronized(ioReg) {
      ...
      if(ioReg.MLS) {
        ...
        if((outBuffer & MOTOR_CONTROL) != 0) {
          ...
          case CARD_END_RECORD | CARD_END_LOAD | CARD_IN | CARD_UNPROTECTED:
            try {
              ...
              if(debug)
                ioReg.console.append("HP9800 MC write " + Integer.toHexString((~outBuffer) & CARD_DATA) + "\n");
            } catch (IOException e) {
              e.printStackTrace();
            }
          }
        ...
        if(debug)
          ioReg.console.append("HP9800 MLS " + Integer.toHexString(value) + "\n");
        // reset MLS pulse
        ioReg.MLS = false;
      }
      ...
      if(ioReg.MCR) {
        ...
        if(debug)
          ioReg.console.append("HP9800 MCR "  + Integer.toHexString(value) + "\n");
      }
  ...
  public boolean input()
  {
    if(!asyncMode && readMode && cardFile != null) {
      ...
      try {
        ...
        if(debug)
          ioReg.console.append("HP9800 MC read " + Integer.toHexString(ioReg.bus.din & CARD_DATA) + "\n");
      } catch (IOException e) {
        // when card reading is complete stop read mode
        readMode = false;
      }
