...
public class HP9820DisplayInterface extends IOinterface implements DisplayInterface
{
  ...
  public boolean output()
  {
    ...
    synchronized(ioReg) {
      ...
      if(ioReg.DEN) {
        // quadrant number
        chr = (ioReg.getValue() & 0x3000) >> 12;

        // column number
        col = ((ioReg.getValue() & 0x0f80) >> 7);
        if(col >= 16)
          col -= 6;

        // get LED column data
        dots = ioReg.getValue() & 0x007f;
        ...
        /*
         * With last column of last character wait 32ms
        ...
        if(col == 4) {
          try {
            if(chr == 15 && !ioReg.dispSRQ) {
              ioReg.wait(ioReg.time_32ms);
            } else
              Thread.yield();
