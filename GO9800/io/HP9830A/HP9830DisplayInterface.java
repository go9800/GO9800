...
public class HP9830DisplayInterface extends IOinterface implements DisplayInterface
{
  ...
  public HP9830DisplayInterface(HP9800Mainframe mainframe)
  {
    super(0);
  ...
  public boolean output()
  {
    ...
    synchronized(ioReg) {
      ...
      if(ioReg.DEN) {
        ...
        // get 2 characters
        c1 = (ioReg.getValue() & 0xfc00) >> 10;
        c2 = (ioReg.getValue() & 0x03f0) >> 4;
        ...
        if(!equal) {
          ...
          mainframe.display(0, pos);
          mainframe.display(0, pos+16);
        }
        ...
        /*
         * With last character wait 16ms for approximate realistic display timing.
        ...
        try {
          if(pos == 15 && !ioReg.dispSRQ) {
            ioReg.wait(ioReg.time_16ms);
          } else
            Thread.yield();
